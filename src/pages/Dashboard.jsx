      ...
      <div className="card" style={{ marginBottom: 20 }}>
        ...
        <div style={{
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            let bgStyle = {};
            if (hasBooked) {
              bgStyle = { background: "var(--color-success-soft)" };
            } else if (hasProposed) {
              bgStyle = { background: "var(--color-warning-soft)" };
            }
            ...
            return (
              <div
                ...
                onMouseOver={(e) => {
                  if (!hasEvents) e.currentTarget.style.background = "var(--color-bg-hover)";
                }}
                onMouseOut={(e) => {
                  if (!hasEvents) e.currentTarget.style.background = "";
                  else if (hasBooked) e.currentTarget.style.background = "var(--color-success-soft)";
                  else if (hasProposed) e.currentTarget.style.background = "var(--color-warning-soft)";
                }}
              >
                <div style={{ 
                  fontSize: 14, 
                ...
                {hasEvents && (
                  <div style={{
                    ...
                    borderRadius: "50%",
                    marginTop: 2,
                    background: hasBooked ? "var(--color-success)" : "var(--color-warning)",
                  }} />
                )}
              </div>
            );
        ...
        <div className="calendar-legend">
          <div className="calendar-legend-item">
            <div className="calendar-legend-dot calendar-legend-dot--proposed" />
            <span>Proposed</span>
          </div>
          <div className="calendar-legend-item">
            <div className="calendar-legend-dot calendar-legend-dot--booked" />
            <span>Booked</span>
          </div>
        </div>
      </div>
              ...
              .map((ev) => {
                ...
                return (
                  <Link
                    key={ev.id}
                    to={`/event/${ev.id}`}
                    className={`event-list-item ${myStatus === 'available' ? 'event-list-item--attending' : ''}`}
        ...
        <div className="modal-backdrop" onClick={() => setShowDayModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button className="btn btn-ghost" style={{ flex: isAdmin ? 0 : 1 }} onClick={() => setShowDayModal(false)}>
                Close
              </button>
            </div>
          </div>
